...
if($submit) {
	...
	$post['email'] = $post['email']? $post['email']: $user['email'];
	$post['passport'] = $post['passport']? $post['passport']: $user['passport'];
	$post['mobile'] = $post['mobile']? $post['mobile']: $user['mobile'];
	...
	if($post['passport'] != $user['passport']){
		if(is_passport($post['passport']) || empty($post['passport'])){
			message('昵称已存在，或不合法！');
		}
	}
...
//		echo '<pre>';var_dump($post['password']);echo '</pre>';exit;
